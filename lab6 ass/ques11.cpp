...
// Function to draw downward parabola using midpoint algorithm
void drawParabola(float a) {
    ...
    while (x <= 2 * a) {  // Arbitrarily decide to limit to 2a along x
        ...
        if (p < 0) {
            ...
            x += 1;
            y -= 1;  // Decrement y since the parabola opens downwards
            p += 2 * x + 1 - 4 * a;
        }
    }

    // Second region where slope > -1 (y decrement is faster)
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Downward Parabola Drawing");  // Create window with title
